<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <h2 class="text-lg font-bold text-gray-900 dm">Edit Pengurus</h2>
            </div>
    ...
    <style>
        ...
        .toggle-group { background: linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%); }
        ...
        /* Edit mode: amber accent for "changed" state */
        .card-field.changed {
        ...
        <div class="max-w-5xl mx-auto">
            ...
            <div class="flex items-center justify-between mb-8 pb-6 border-b border-slate-200/70">
                <div class="flex items-center gap-2">
                    ...
                    <p class="text-xs text-slate-500 font-medium">Mengubah data: <span class="font-semibold text-slate-700">{{ $pengurus->nama }}</span></p>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start w-full">
                ...
                <div class="lg:col-span-1">
                    <form action="{{ route('admin.pengurus.update', $pengurus->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="flex-1 min-w-0 w-full">
                                    ...
                                    <select name="organisasi_otonom_id" required
                                        ...
                                        @foreach($organisasis as $org)
                                            <option value="{{ $org->id }}"
                                                {{ (old('organisasi_otonom_id', $pengurus->organisasi_otonom_id) == $org->id) ? 'selected' : '' }}>
                                                {{ $org->nama }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="card-field au d2 bg-white/90 backdrop-blur-sm border border-slate-200/70 rounded-2xl p-6 shadow-lg hover:shadow-xl">
                            <div class="flex items-start gap-4 relative z-10">
                                ...
                                <div class="flex-1 min-w-0 w-full">
                                    ...
                                    <input type="text" name="nama" value="{{ old('nama', $pengurus->nama) }}" maxlength="200" required
                            ...
                            <div class="card-field bg-white/90 backdrop-blur-sm border border-slate-200/70 rounded-2xl p-6 shadow-lg hover:shadow-xl">
                                <div class="flex items-start gap-4 relative z-10">
                                    ...
                                    <div class="flex-1 min-w-0 w-full">
                                        ...
                                        <input type="text" name="jabatan" value="{{ old('jabatan', $pengurus->jabatan) }}" maxlength="100" required
                                ...
                                <div class="flex items-start gap-4 relative z-10">
                                    ...
                                    <div class="flex-1 min-w-0 w-full">
                                        ...
                                        <select name="level" required
                                            ...
                                            <option value="inti"    {{ old('level', $pengurus->level) == 'inti'    ? 'selected' : '' }}>Inti</option>
                                            <option value="majelis" {{ old('level', $pengurus->level) == 'majelis' ? 'selected' : '' }}>Majelis</option>
                                            <option value="lembaga" {{ old('level', $pengurus->level) == 'lembaga' ? 'selected' : '' }}>Lembaga</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card-field au d4 bg-white/90 backdrop-blur-sm border border-slate-200/70 rounded-2xl p-6 shadow-lg hover:shadow-xl">
                            <div class="flex items-start gap-4 relative z-10">
                                ...
                                <div class="flex-1 min-w-0 w-full">
                                    ...
                                    <input type="text" name="bidang" value="{{ old('bidang', $pengurus->bidang) }}" maxlength="100"
                            ...
                            <div class="card-field bg-white/90 backdrop-blur-sm border border-slate-200/70 rounded-2xl p-6 shadow-lg hover:shadow-xl">
                                <div class="flex items-start gap-4 relative z-10">
                                    ...
                                    <div class="flex-1 min-w-0 w-full">
                                        ...
                                        <input type="number" name="periode_mulai" value="{{ old('periode_mulai', $pengurus->periode_mulai) }}" required
                            ...
                            <div class="card-field bg-white/90 backdrop-blur-sm border border-slate-200/70 rounded-2xl p-6 shadow-lg hover:shadow-xl">
                                <div class="flex items-start gap-4 relative z-10">
                                    ...
                                    <div class="flex-1 min-w-0 w-full">
                                        ...
                                        <input type="number" name="periode_selesai" value="{{ old('periode_selesai', $pengurus->periode_selesai) }}" required
                            ...
                            <div class="card-field bg-white/90 backdrop-blur-sm border border-slate-200/70 rounded-2xl p-6 shadow-lg hover:shadow-xl">
                                <div class="flex items-start gap-4 relative z-10">
                                    ...
                                    <div class="flex-1 min-w-0 w-full">
                                        ...
                                        <input type="text" name="no_hp" value="{{ old('no_hp', $pengurus->no_hp) }}" maxlength="20"
                            ...
                            <div class="card-field bg-white/90 backdrop-blur-sm border border-slate-200/70 rounded-2xl p-6 shadow-lg hover:shadow-xl">
                                <div class="flex items-start gap-4 relative z-10">
                                    ...
                                    <div class="flex-1 min-w-0 w-full">
                                        ...
                                        <input type="email" name="email" value="{{ old('email', $pengurus->email) }}" maxlength="100"
                            ...
                            <div class="flex items-start gap-4 relative z-10">
                                ...
                                <div class="flex-1 min-w-0 w-full">
                                    ...
                                    <p class="text-xs text-slate-400 mt-1">
                                        {{ $pengurus->foto ? 'Kosongkan jika tidak ingin mengganti foto.' : 'Format: png, jpg | Max: 2MB' }}
                                    </p>
                                    @error('foto')
                                        <p class="text-red-500 text-xs mt-1.5 font-medium">{{ $message }}</p>
                                    @enderror
                            ...
                            <div class="flex items-center justify-between relative z-10">
                                ...
                                <label class="relative z-20 inline-flex items-center cursor-pointer group">
                                    ...
                                    <input type="checkbox" id="status-toggle" name="is_active" value="1"
                                        {{ old('is_active', $pengurus->is_active) ? 'checked' : '' }} class="sr-only toggle-checkbox">
                ...
                <div class="lg:block hidden lg:sticky lg:top-20 space-y-4 lg:col-span-1 lg:ml-8">
                    ...
                    <div class="bg-amber-50/80 backdrop-blur-sm border border-amber-200/60 rounded-2xl p-5 shadow-sm">
                        ...
                        <div class="space-y-1.5 text-xs text-amber-800">
                            <div class="flex justify-between"><span class="text-amber-600">Nama</span><span class="font-medium truncate max-w-[120px]">{{ $pengurus->nama }}</span></div>
                            <div class="flex justify-between"><span class="text-amber-600">Jabatan</span><span class="font-medium">{{ $pengurus->jabatan }}</span></div>
                            <div class="flex justify-between"><span class="text-amber-600">Level</span><span class="font-medium capitalize">{{ $pengurus->level }}</span></div>
                            <div class="flex justify-between"><span class="text-amber-600">Periode</span><span class="font-medium">{{ $pengurus->periode_mulai }}–{{ $pengurus->periode_selesai }}</span></div>
                            <div class="flex justify-between"><span class="text-amber-600">Status</span>
                                <span class="font-medium {{ $pengurus->is_active ? 'text-emerald-600' : 'text-red-500' }}">
    ...
    <script>
        ...
        function updateToggleVisual(checkbox) {
            ...
            const toggleSlider = label ? label.querySelector('#toggle-slider') : document.getElementById('toggle-slider');
            const toggleKnob   = label ? label.querySelector('#toggle-knob')   : document.getElementById('toggle-knob');

            if (checkbox.checked) {
                if (toggleSlider) { toggleSlider.classList.add('bg-emerald-600'); toggleSlider.classList.remove('bg-slate-200'); }
                if (toggleKnob)   toggleKnob.style.transform = 'translateX(20px)';
            } else {
                if (toggleSlider) { toggleSlider.classList.remove('bg-emerald-600'); toggleSlider.classList.add('bg-slate-200'); }
        ...
        function updateCompletion() {
            ...
            if (footerStatus) {
                footerStatus.textContent = remaining === 0
                    ? '✓ Semua field wajib terisi'
                    : remaining + ' field wajib tersisa';
            }

            // Sync filled badges on init (for pre-filled values)
            syncAllBadges();

            updateProgressChecklist({
                organisasi: organisasiSelect && organisasiSelect.value !== '',
                nama:       namaInput && namaInput.value.trim() !== '',
                jabatan:    jabatanInput && jabatanInput.value.trim() !== '',
                level:      levelSelect && levelSelect.value !== '',
                periode:    periodeMulai && periodeMulai.value.trim() !== '' && periodeSelesai && periodeSelesai.value.trim() !== '',
                status:     statusCheckbox && statusCheckbox.checked,
            });
        }

        function syncAllBadges() {
            // Sync all individual card badges based on their input current value
            document.querySelectorAll('.card-field').forEach(card => {
                const inputs   = card.querySelectorAll('input:not([type="hidden"]):not([type="file"]):not([type="checkbox"]), select');
                const badge    = card.querySelector('.filled-badge');
                if (!badge || inputs.length === 0) return;

                let filled = false;
                inputs.forEach(inp => {
                    if (inp.value && inp.value.trim() !== '') filled = true;
                });
        ...
        function updateProgressChecklist(states) {
            ...
            const keys   = ['organisasi','nama','jabatan','level','periode','status'];
            keys.forEach((key, i) => {
                if (checks[i]) checks[i].classList.toggle('hidden', !states[key]);
            });
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (statusCheckbox) {
                ...
                statusCheckbox.addEventListener('change', function() {
                    ...
                    setTimeout(() => card.classList.remove('focused'), 200);
                });
            }

            // Run on load so pre-filled values are reflected immediately
            updateCompletion();
        });
    </script>
</x-app-layout>
